...
<# Begin: DataSource "AD-user-get-attribute-enabled" #>
$tmpPsScript = @'
$UserPrincipalName = $datasource.selectedUser.UserPrincipalName
Write-information "Searching AD user [$userPrincipalName]"
...
Invoke-HelloIDDatasource -DatasourceName "AD-user-get-attribute-enabled" -DatasourceType "4" -DatasourceInput $tmpInput -DatasourcePsScript $tmpPsScript -DatasourceModel $tmpModel -returnObject ([Ref]$dataSourceGuid_2) 
<# End: DataSource "AD-user-get-attribute-enabled" #>
...
Invoke-HelloIDDynamicForm -FormName "AD Account - (De)Activate" -FormSchema $tmpSchema  -returnObject ([Ref]$dynamicFormGuid) 
<# END: Dynamic Form #>
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD Account - (De)Activate" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-unlock" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-user-set-enabled" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD Account - (De)Activate' already exists. Nothing to do with the Delegated Form task..." 
}
